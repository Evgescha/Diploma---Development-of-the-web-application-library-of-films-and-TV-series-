...
@RequestMapping(GenreController.CURRENT_ADDRESS)
public class GenreController {
    public static final String API_PATH = "genre";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new Genre());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute Genre entity, RedirectAttributes ra) {
